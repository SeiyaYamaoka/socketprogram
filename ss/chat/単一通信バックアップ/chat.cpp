...
BOOL CALLBACK DialogFunc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND:
		...
		case IDD_PB_SEND:
			...
			//AddEdBoxString(hWnd, (LPCSTR)"なにぬんお");
	...
	case SM_ASYNC://ネットワークイベント発生
		...
		if (nErrorCode != 0) { ErrorProc(s, "Errorcode = 0", hWnd); }
		switch (nEventCode) {
		case FD_ACCEPT:
			...
			len = sizeof addr;
			s = accept(sa, (struct sockaddr*)&addr, &len);
			if (s == INVALID_SOCKET) {
			...
			//AddEdBoxString(hWnd, name);
			AddEdBoxString(hWnd, "接続しました。");
			...
			if (WSAAsyncSelect(s, hWnd, SM_ASYNC, FD_READ | FD_WRITE | FD_CLOSE) == SOCKET_ERROR) {
				ErrorProc(s, "WSAAsyncSelect FD_ACCEPT", hWnd);
			}
			break;
...
void ErrorProc(SOCKET sock,const char *ApiName, HWND hWnd) {
	...
	if(WSAGetLastError() != 10035){
		MessageBox(hWnd, (LPCSTR)ApiName, ("Err"), MB_ICONSTOP);
		EndDialog(hWnd, 1);
		closesocket(s);
		closesocket(sa);
